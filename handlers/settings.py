...
async def cb_close_settings(client: Client, cb: CallbackQuery):
    _edit_state.pop(cb.from_user.id, None)
...
async def on_text_input(client: Client, message: Message):
    ...
    async def _save_and_back(msg: str, parse_mode=ParseMode.HTML):
        _edit_state.pop(admin_id, None)
        settings = await get_settings(admin_id)
        await pacing.reply(message, msg,
    ...
    if state == "caption":
        ...
        await _save_and_back("✅ Caption template saved!")
        ...
        await _save_and_back("✅ Button label: <code>" + text + "</code>")
        ...
        try:
            ...
            return await pacing.reply(message,
                "❌ Channel not found: <code>" + str(e) + "</code>",
                parse_mode=ParseMode.HTML)
